...
if st.sidebar.button("Add/Update Scale"):
    try:
        ...
        if px_val != 0:
            um_to_px_map[selected_um] = um_val / px_val
            save_scale_settings(um_to_px_map)
            st.sidebar.success(f"Scale {selected_um} µm = {um_val / px_val:.6f} µm/px saved.")
    except ValueError:
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        for region in region_props:
            ...
            crystal_to_cell.append({
                ...
                "Region_Area (µm²)": region.area * (um_to_px_map ** 2)um_to_px_map
            })
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        for region in region_props:
            ...
            crystal_to_cell.append({
                ...
                "Overlap (pixels)": max_overlap,
                "Region_Area (pixels)": region.area,
                "Region_Area (µm²)": region.area * (um_to_px_map ** 2)
            })
