...
# Save scale settings
def save_scale_settings(um_to_px_map):
    with open('scale_map.json', 'w') as f:
        json.dump(um_to_px_map, f)

um_to_px_map = load_scale_settings()

# Input: Known scale selection and entry
...
if st.sidebar.button("Add/Update Scale"):
    try:
        um_val = float(selected_um)
        px_val = float(distance_in_px)
        if px_val != 0:
            um_to_px_map[selected_um] = 1/(um_val / px_val)
            save_scale_settings(um_to_px_map)
            st.sidebar.success(f"Scale {selected_um} µm = {1/(um_val / px_val):.6f} µm/px saved.")
    except ValueError:
        st.sidebar.error("Invalid numeric input.")
...
# Start Button
if st.button("Number of crystals"):
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        for region in region_props:
            ...
            crystal_to_cell.append({
                ...
                "Region_Area (µm²)": region.area * (px_per_um ** 2)
            })
...
if st.session_state.script1_results:
    ...
    for result1 in st.session_state.script1_results:
        ...
        st.image(result1["annotated_path"], caption="Segmented Image")
        st.image(result1["overlap_path"], caption="Overlap Image")
        st.image(result1["annotated_image_path"], caption="Detections")
        st.image(result1["hist_path_Areas"], caption="Areas Histogram")
        st.image(result1["hist_A_path"], caption="Pixels Intensity Histogram")
...
if st.session_state.script2_results:
    ...
    for result2 in st.session_state.script2_results:
        ...
        st.image(result2["annotated_path"], caption="Segmented Image")
        st.image(result2["overlap_path"], caption="Overlap Image")
        st.image(result2["annotated_image_path"], caption="Detections")
        st.image(result2["hist_path_Areas"], caption="Areas Histogram")
        st.image(result2["hist_A_path"], caption="Pixels Intensity Histogram")
