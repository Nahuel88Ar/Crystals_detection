...
#------------------------------------------------------------------------------------------
        ...
        # Calculate total cells area (µm²) from your existing DataFrame
        total_area = region_area_df["Region_Area (µm²)"].sum()

        # Calculate total crystals area (µm²) from df_mapping (crystals)
        total_area_cr = df_mapping["Region_Area (µm²)"].sum()

        # Calculate percentage of crystal area relative to cell area
        Percentage = (total_area_cr / total_area) * 100 if total_area > 0 else 0
    ...
    summary_excel_path = os.path.join(output_dir, "Summary_Cells_Crystals_Areas.xlsx")
    summary_df.to_excel(summary_excel_path, index=False)

    # Show in Streamlit
    st.header("Summary of Cells and Crystals Areas")
    st.dataframe(summary_df)

    # Provide a download button
    with open(summary_excel_path, "rb") as f3:
        st.download_button("Download Summary Dataset", f3, file_name="Summary_Cells_Crystals_Areas.xlsx")
