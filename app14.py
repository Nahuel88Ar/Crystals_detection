...
if st.session_state.script1_done:
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        if "Region_Area (µm²)" in df_mapping.columns:
            ...
            st.warning("⚠️ 'Region_Area (µm²)' column not found in df_mapping. Skipping area filter.")
        ...
        if "Associated_Cell" in df_mapping.columns:
            ...
            st.warning("⚠️ 'Associated_Cell' column not found in df_mapping. Skipping count calculations.")
            df_mapping["Associated_Cell_Count"] = 0
            df_mapping["Total_Cells_with_crystals"] = 0

        total_area_crystals = df_mapping.get("Region_Area (pixels)", pd.Series()).sum() * (PIXEL_TO_UM ** 2)
        df_mapping.loc[len(df_mapping)] = ["", "", "", "Total Area Crystals", total_area_crystals, "", ""]
        ...
        if "Associated_Cell" in df_mapping.columns and not df_mapping["Associated_Cell"].isna().all():
            ...
            st.warning("⚠️ Merge skipped: 'Associated_Cell' column is missing or all values are NaN.")
