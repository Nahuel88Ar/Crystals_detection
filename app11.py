...
if st.session_state.script1_done:
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        if grayA is None:
            st.warning(f"Failed to convert {bf_file.name} to grayscale. Skipping...")
            continue
...
#------------------------------------------------------------------------------------------
        ...
        # Black out bottom-right region (scale bar)
        #grayA[h - crop_margin_h:, w - crop_margin_w:] = 0.0
        ...
        for region in region_props_A:
            if region.area < average:
                ...
                # Get coordinates
                coordinates = peak_local_max(distance, labels=region_mask, min_distance=10)
        ...
        # Add to summary
        summary_rows.append({
            #"file_numeric_id": file_numeric_id,
...
#------------------------------------------------------------------------------------------
        ...
        # ✅ Fixed and active version
        if not df_mapping.empty and "Region_Area (µm²)" in df_mapping.columns and "Overlap (pixels)" in df_mapping.columns:
            ...
            print("Skipping filtering: df_mapping is empty or missing expected columns.")
            total_area_cr = 0
    ...
    plt.title("Average % área Crystals/Cells per day", fontsize=14)
    plt.xlabel("DAY (DAYS)", fontsize=12)
    ...
    # Exportar también los datos agrupados
