...
class ImageProcessingApp(QWidget):
    ...
    def start_processing(self):
        ...
        # Start file indexing at 1 for labeling outputs consistently
        file_numeric_id = 1
        ...
        for bf_file, pl_file in zip(bf_files, pl_files):
            ...
            # Convert PL (polarized light) image to grayscale and enhance local contrast using adaptive histogram equalization
            grayB = rgb2gray(imageB)
            grayB = exposure.equalize_adapthist(grayB)

            # Apply bilateral filter to reduce noise in PL grayscale image
            ...
            dynamic_threshold = mean_intensity + 4 * std_intensity  # Threshold = mean + 4×std deviation

            # Generate binary mask of crystals using the dynamic threshold
            binary_B = (grayB > dynamic_threshold).astype(np.uint8)

            # Plot and save histogram of PL intensities with threshold marked
            plt.figure(figsize=(8, 6))
            ...
            # Resize both masks to match dimensions for overlap analysis
            ...
            # Calculate overlap: where both cell and crystal masks are positive
            overlap = (np.logical_and(filtered_binary_A_resized > 0, binary_B_resized > 0)).astype(np.uint8) * 255

            # Mask the scale bar area (bottom-right corner) from overlap results
            h2, w2 = overlap.shape
            overlap[h2-60:h2, w2-450:w2] = 0  # Manually exclude scale bar region

            # Save the binary overlap image
            ...
            # Map overlapping regions (crystals) to corresponding cells
            ...
            # Assign each crystal region to the cell with the largest overlapping area
            ...
            # Save all results into a multi-sheet Excel file
            ...
            # ------------------- Visualization -------------------
            # Draw rectangles around crystals and annotate cell matches
            ...
            # Show image and overlap side by side
            ...
            # Initialize summary list (if this is inside a per-image loop, it's re-initialized for each file)
            summary_rows = []
    ...
    def start_processing_2(self):
        ...
        for bf_file, pl_file in zip(bf_files, pl_files):
            ...
            mean_intensity = np.mean(grayB)
            std_intensity = np.std(grayB)
            dynamic_threshold = mean_intensity + 4.6 * std_intensity  # Threshold = mean + 4.6×std deviation
        ...
        # Plot average percentage of crystal area per day
        plt.figure(figsize=(10, 6))
        plt.plot(
            grouped_df["DAYS"],
            grouped_df["mean_percentage"],
        ...
        # Add shaded area showing ±1 standard deviation
        plt.fill_between(
            grouped_df["DAYS"],
            grouped_df["mean_percentage"] - grouped_df["std_percentage"],
            grouped_df["mean_percentage"] + grouped_df["std_percentage"],
        ...
        plt.title("Average % Area Crystals/Cells per Day", fontsize=14)
        plt.xlabel("Day", fontsize=12)
        plt.ylabel("% Area Crystals / Cells", fontsize=12)
        plt.ylim(0, y_max_limit)
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        ...
        # Optionally remove the original files after archiving
        for file_path in all_output_files:
            if os.path.exists(file_path):
                os.remove(file_path)
