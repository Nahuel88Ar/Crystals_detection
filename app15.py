...
if st.session_state.script1_done:
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        for region in region_props:
            ...
            crystal_to_cell.append({
                ...
                "Associated_Cell": best_match_cell if best_match_cell is not None else -1,
                "Overlap (pixels)": max_overlap,
                "Region_Area (pixels)": region.area,
                "Region_Area (µm²)": region.area * (PIXEL_TO_UM ** 2) if PIXEL_TO_UM else 0.0
            })
        ...
        st.write("📋 df_mapping head:")
        st.write(df_mapping.head())
        st.write("📋 df_mapping columns:", df_mapping.columns.tolist())

        if "Region_Area (µm²)" in df_mapping.columns and "Overlap (pixels)" in df_mapping.columns:
            df_mapping = df_mapping[
                (df_mapping["Region_Area (µm²)"] < 10) & 
                (df_mapping["Overlap (pixels)"] > 0)
    ]
        else:
            st.warning("❗ Columns 'Region_Area (µm²)' or 'Overlap (pixels)' not found in df_mapping.")
            st.write("📋 Available columns:", df_mapping.columns.tolist())

        if "Associated_Cell" in df_mapping.columns:
            # Filter out any bad rows BEFORE mapping (optional safety)
            df_mapping_numeric = df_mapping[df_mapping["Associated_Cell"].apply(lambda x: isinstance(x, (int, float)))]
    
            # Create counts only from numeric Associated_Cell values
            counts = df_mapping_numeric["Associated_Cell"].value_counts()
            df_mapping["Associated_Cell_Count"] = df_mapping["Associated_Cell"].map(counts)

            df_mapping["Total_Cells_with_crystals"] = df_mapping_numeric["Associated_Cell"].nunique()

            # ⛔ ONLY add this at the end, once everything else is done
            df_mapping.loc["Total"] = ["", "", "", "Total Area Crystals", df_mapping["Region_Area (µm²)"].sum(), "", ""]
        else:
            st.warning("❗ Column 'Associated_Cell' not found in df_mapping.")
            st.write("📋 Available columns:", df_mapping.columns.tolist())
    ...
    # Limitar el eje Y al máximo valor +1% (hasta 100)
    ...
    # Agregar banda de desviación estándar (opcional)
    plt.fill_between(
        grouped_df["DAYS"],
    ...
    plt.title("Mean % of Cells with Crystals per Day")
    plt.xlabel("Day")
...
if st.session_state.script1_results:
    ...
    for idx, result1 in enumerate(st.session_state.script1_results):
        ...
        st.image(result1["plot_path"], caption="📈 Percentage of Cells with Crystals")
...
if st.session_state.script2_results:
    ...
    for idx, result2 in enumerate(st.session_state.script2_results):
        ...
        st.image(result2["annotated_img_path"], caption="Detections crystals")
    ...
    with open(excel_path_2, "rb") as f3:
        st.download_button(
            "📊 Download Dataset Plot",
            f3,
            file_name=os.path.basename(excel_path_2),
