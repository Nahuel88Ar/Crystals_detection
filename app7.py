...
# File Upload
bf_zip = st.file_uploader("Upload BF Images ZIP (.zip)", type=["zip"])
pl_zip = st.file_uploader("Upload PL Images ZIP (.zip)", type=["zip"])

bf_files = []
pl_files = []

if bf_zip is not None:
    with zipfile.ZipFile(bf_zip) as bf_archive:
        bf_files = [f for f in bf_archive.namelist() if f.lower().endswith('.tif')]
        bf_files.sort()
        bf_temp_dir = tempfile.mkdtemp()
        for file in bf_files:
            bf_archive.extract(file, bf_temp_dir)
        bf_files = [os.path.join(bf_temp_dir, f) for f in bf_files]

if pl_zip is not None:
    with zipfile.ZipFile(pl_zip) as pl_archive:
        pl_files = [f for f in pl_archive.namelist() if f.lower().endswith('.tif')]
        pl_files.sort()
        pl_temp_dir = tempfile.mkdtemp()
        for file in pl_files:
            pl_archive.extract(file, pl_temp_dir)
        pl_files = [os.path.join(pl_temp_dir, f) for f in pl_files]

# Sort uploaded files
if bf_files:
    bf_files = sorted(bf_files)
if pl_files:
    pl_files = sorted(pl_files)
...
if bf_files and pl_files:
    ...
    for bf, pl in zip(bf_files, pl_files):
        st.write(f"Processing: {os.path.basename(bf)} and {os.path.basename(pl)}")
#------------------------------------------------------------------------------------------------------------------------------------------------------#
# Output Directory
output_dir = "outputs"
...
if st.session_state.script3_done:
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        # Save the annotated image
        annotated_path = os.path.join(output_dir, f"{os.path.basename(bf_file)}_Segmented_Annotated.png")
        cv2.imwrite(annotated_path, overlay_image)
        all_output_files.append(annotated_path)
